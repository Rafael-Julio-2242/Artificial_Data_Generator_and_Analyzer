...
func CalculateFrequencies(context *gin.Context) {
	...
	calculateFrequenciesServiceSv, ok := context.Get(services.CalculateFrequenciesServiceKey)

	if !ok {
		context.JSON(500, gin.H{"message": "error getting calculate frequencies service"})
		return
	}

	calculateFrequenciesService := calculateFrequenciesServiceSv.(services.CalculateFrequencies)

	frequenciesData, err := calculateFrequenciesService.Calculate(fixedData)

	if err != nil {
		context.JSON(500, gin.H{"message": "error calculating frequencies: " + err.Error()})
		return
	}
	...
	if csvReturn {
		...
		buf, err := csvGenerator.GenerateCSV(frequenciesData)
	...
	context.JSON(http.StatusOK, frequenciesData)
